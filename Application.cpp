...
int main()
{
    ...
    if (!BeeHive::window.self) return EXIT_FAILURE;
    ...
    std::vector<Entity*> figuras = {&cajita, &otra_caja};
    ...
    while (!glfwWindowShouldClose(BeeHive::window.self))
    {
        ...
        camara.processInput(BeeHive::window.self, clock.deltaTime);
        if (BeeHive::mouse.hasUnprocesedOffsets())
            camara.processMouseInput(BeeHive::mouse.xOffset, BeeHive::mouse.yOffset);
        // Start the Dear ImGui frame
        ImGui_ImplOpenGL3_NewFrame();
        ImGui_ImplGlfw_NewFrame();
        ImGui::NewFrame();
        //HiveEngine GUI (hay que poner esto en una funcion aparte)
        if (ImGui::BeginMainMenuBar())
        {
            if (ImGui::BeginMenu("Nuevo"))
            {
                if (ImGui::MenuItem("Caja"))
                {
        ...
        ImVec2 pos((float)BeeHive::window.viewport_width, 0.0f);
        ImVec2 size(BeeHive::window.width - BeeHive::window.viewport_width, BeeHive::window.viewport_heigt);
        ImGui::SetNextWindowPos(pos);
        ImGui::SetNextWindowSize(size);
        if (ImGui::Begin("Editor", NULL, flags))
        {
            BeeHive::cameraMenu(&camara,"Mi Camara");
            int i = 0;
            for (auto v: figuras)
            {
                i++;
                ImGui::PushID(i);
                BeeHive::entityMenu(v,"Figura");
                ImGui::PopID();
            } 
        }
        ImGui::End();
        ...
        glfwSwapBuffers(BeeHive::window.self);
    }
